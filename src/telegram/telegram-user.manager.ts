...
@Injectable()
export class TelegramUserManager implements OnApplicationBootstrap {
  ...
  async onApplicationBootstrap() {
    await this.init().catch(e => console.error('[TgUser] init failed:', e.message));
  }
  ...
  async init() {
    ...
    const active = await this.prisma.telegramUserAuth.findMany({
      where: { status: 'ACTIVE' },
  ...
  async verify(channelId: string, code: string, phoneCodeHash: string, password?: string) {
    ...
    await this.prisma.telegramUserAuth.update({
      where: { channelId },
  ...
  async startQrAuth(
    ...
    const upserted = await this.prisma.telegramUserAuth.upsert({
      where: { channelId },
      update: { sessionString: encryptSession(rawSession), status: 'ACTIVE', phoneNumber },
      create: {
        channelId,
        phoneNumber,
        apiId: this.API_ID,
  ...
  async sendMessage(channelId: string, chatId: string, text: string) {
    ...
    this.postOutgoing(channelId, chatId, result.id, text, result.date);
  ...
  async startConversation(channelId: string, target: string, text: string) {
    ...
    this.postOutgoing(channelId, chatId, result.id, text, result.date, name, username);
  ...
  private postOutgoing(
    ...
    const NEXT_URL = process.env.TG_NEXT_URL!;
    const SECRET = process.env.INTERNAL_WEBHOOK_SECRET!;
    fetch(`${NEXT_URL}/api/telegram/userbot/webhook/${channelId}`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', 'x-webhook-secret': SECRET },
      body: JSON.stringify({ chatId, messageId, text, senderName: null, chatTitle, chatUsername, date, direction: 'OUT' }),
    }).catch(e => console.error(`[TgUser] Webhook OUT failed for ${channelId}:`, e));
  }
}
